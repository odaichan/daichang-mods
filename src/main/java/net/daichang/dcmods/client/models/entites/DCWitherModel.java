package net.daichang.dcmods.client.models.entites;
...
@OnlyIn(Dist.CLIENT)
public class DCWitherModel <T extends DCWither> extends HierarchicalModel<T> {
    ...
    public DCWitherModel(ModelPart pRoot) {
        this.root = pRoot;
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        $$2.addOrReplaceChild("shoulders", CubeListBuilder.create().texOffs(0, 16).addBox(-10.0F, 3.9F, -0.5F, 20.0F, 3.0F, 3.0F, new CubeDeformation(0.0F)), PartPose.ZERO);
        float $$3 = 0.20420352F;
        ...
        $$2.addOrReplaceChild("tail", CubeListBuilder.create().texOffs(12, 22).addBox(0.0F, 0.0F, 0.0F, 3.0F, 6.0F, 3.0F, new CubeDeformation(0.0F)), PartPose.offsetAndRotation(-2.0F, 6.9F + Mth.cos(0.20420352F) * 10.0F, -0.5F + Mth.sin(0.20420352F) * 10.0F, 0.83252203F, 0.0F, 0.0F));
        $$2.addOrReplaceChild("center_head", CubeListBuilder.create().texOffs(0, 0).addBox(-4.0F, -4.0F, -4.0F, 8.0F, 8.0F, 8.0F, new CubeDeformation(0.0F)), PartPose.ZERO);
        CubeListBuilder $$4 = CubeListBuilder.create().texOffs(32, 0).addBox(-4.0F, -4.0F, -4.0F, 6.0F, 6.0F, 6.0F, new CubeDeformation(0.0F));
    ...
    public void setupAnim(DCWither pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        float $$6 = Mth.cos(pAgeInTicks * 0.1F);
    ...
    public void renderToBuffer(PoseStack pPoseStack, VertexConsumer pBuffer, int pPackedLight, int pPackedOverlay, float pRed, float pGreen, float pBlue, float pAlpha) {
        ...
        ribcage.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pRed, pGreen ,pBlue, pAlpha);
        tail.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pRed, pGreen ,pBlue, pAlpha);
        centerHead.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pRed, pGreen ,pBlue, pAlpha);
    ...
    public void prepareMobModel(DCWither pEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick) {
        setupHeadRotation(pEntity, this.rightHead, 0);
        setupHeadRotation(pEntity, this.leftHead, 1);
    }

    private static <T extends DCWither> void setupHeadRotation(DCWither pWither, ModelPart pPart, int pHead) {
