...
public class DCDiscFont extends Font {
    public DCDiscFont(Function<ResourceLocation, FontSet> p_243253_, boolean p_243245_) {
        super(p_243253_, p_243245_);
    }

    public static DCDiscFont getFont() {
        return new DCDiscFont(Minecraft.getInstance().font.fonts, false);
    }
    ...
    public int renderFont(@NotNull String text, float x, float y, int rgb, boolean dropShadow, @NotNull Matrix4f matrix4f, @NotNull MultiBufferSource multiBufferSource, @NotNull DisplayMode mode, int i, int i1, boolean isText){
        for (int index = 0; index < text.length(); index++) {
            ...
            int color1 = rgb & 0xFFFFD8D8;
            int color2 = rgb & 0xFFFF9999;
