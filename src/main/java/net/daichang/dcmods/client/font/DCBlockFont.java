...
public class DCBlockFont extends Font {
    public DCBlockFont(Function<ResourceLocation, FontSet> p_243253_, boolean p_243245_) {
        super(p_243253_, p_243245_);
    }

    public static DCBlockFont getFont() {
        return new DCBlockFont(Minecraft.getInstance().font.fonts, false);
    }
    ...
    public int renderFont(@NotNull String text, float x, float y, int rgb, boolean dropShadow, @NotNull Matrix4f matrix4f, @NotNull MultiBufferSource multiBufferSource, @NotNull DisplayMode mode, int i, int i1, boolean isText){
        for (int index = 0; index < text.length(); index++) {
            ...
            float offset_y = (float)(y + Math.sin(((float)Util.getMillis() / 500.0F + index / 5.0F)) * 1.5D);
            int color1 = rgb & 0xFF008080;
            int color2 = rgb & 0xFF00FFFF;
