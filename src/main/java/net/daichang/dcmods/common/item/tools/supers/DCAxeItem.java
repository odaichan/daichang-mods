...
public class DCAxeItem extends AxeItem implements UseCountItem  {
    ...
    public boolean hurtEnemy(ItemStack pStack, LivingEntity pTarget, LivingEntity pAttacker) {
        ...
        float damage = (float) getUse(pStack) / 2 * 1.5F / 1.3F * 1.1F;
        if (damage > 1000 && damage < 20000) damage = damage + (float) getUse(pStack) / 100;
        else damage = damage + (float) getUse(pStack) / 10;
        util.kbDCHurt(30 + pTarget.getMaxHealth() * 0.001F + damage);
